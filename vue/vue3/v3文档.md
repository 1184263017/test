...
1. 定义事件 defineEmits([事件名称,...]) 
- 参数:参数是事件名称,使用数组定义多个事件
- 返回值:返回一个函数`emit = defineEmits()`

2. 触发事件
-  使用`emit`函数,触发事件
- `emit('事件名称')` 触发事件处理函数
- `emit('事件名称',参数)` 参数将传递给事件处理函数

## 组件 v-model

1. `v-model:参数名称`,参数名称是`modelValue`,modelValue可以省略不写
2. `v-model:参数名称`,相当于通过 `:参数名称=值` 实现父子 传参,通过绑定 `@update:参数名称` 事件,实现子父传参

### 处理 v-model 修饰符

1. `v-model:参数名称.修饰符1.修饰符2` 本质上就是指令修饰符
2. 在子组件中可以通过`props.参数名称Modifiers` 对象获取,该对象收集了指令所有的修饰符
...
1. provide  提供“变量”,此“变量”可以被所有的后代组件访问
2. inject   获取“变量”,遵循就近原则,从最近的上级组件中获取(注入)“变量”,第二个参数是默认值

### 和响应是数据配合使用

1. 供给方的数据必须是一个对象,对象中必须包含两个属性,第一个属性是数据本身,第二个属性是修改属性的方法
2. 一般来说,数据属性命名和变量名一直,修改数据的方法名`update变量名`
3. 约定在子组件中修改使用update方法修改变量,不要直接修改
4. 可以使用`readonly`函数包装下数据,包装后的数据只能读不能写
5. 可以使用一个js文件将所有的注入名称定义到这个js中,避免注入名称重名(大型项目使用)
